...
- [CPlusplus-Interview](#cplusplus-interview)
  - [C++基础](#c基础)
    ...
    - [5.  说一下C++代码文件到可执行程度，经历了哪些过程？](#5--说一下c代码文件到可执行程度经历了哪些过程)
    ...
    - [33 . `typedef`和`using`的区别？](#33--typedef和using的区别)
    - [34 . 说一下在函数中不同位置使用const的情况，并解释不同位置下，const的作用？](#34--说一下在函数中不同位置使用const的情况并解释不同位置下const的作用)
    - [35 . 什么内联函数，说一下`inline`的优缺点，它和宏定义有什么区别？](#35--什么内联函数说一下inline的优缺点它和宏定义有什么区别)
    - [36 . `struct`和`class`的区别](#36--struct和class的区别)
    - [37 . `explicit` 的作用，为什么要用它](#37--explicit-的作用为什么要用它)
    - [38 . 说一下友元的应用场景，它会有什么问题](#38--说一下友元的应用场景它会有什么问题)
    - [39 . 解释一下C++中的类型转换](#39--解释一下c中的类型转换)
    - [40 . 介绍一下RAII](#40--介绍一下raii)
    - [41 . const常量在C和C++中有什么区别](#41--const常量在c和c中有什么区别)
    - [42 . 说一下C++模板的偏特化与全特化它们的应用场景](#42--说一下c模板的偏特化与全特化它们的应用场景)
    - [43 . 解释一下C++中的内存对齐？](#43--解释一下c中的内存对齐)
    - [44 . 解释一下`volatile`关键字](#44--解释一下volatile关键字)
    - [45 . 浅拷贝和深拷贝有什么区别？](#45--浅拷贝和深拷贝有什么区别)
    - [46 . `sizeof`和`strlen`有什么区别？](#46--sizeof和strlen有什么区别)
    - [47 . 说一下类的初始化列表，为什么说它可以提高效率?](#47--说一下类的初始化列表为什么说它可以提高效率)
    - [48 . 内存泄漏的场景一般有哪些情况](#48--内存泄漏的场景一般有哪些情况)
    - [49 . 解释一下`constexpr`关键字？](#49--解释一下constexpr关键字)
    - [50 . `const`和`constexpr`有什么区别？](#50--const和constexpr有什么区别)
    - [51 . `enum`和`enum class`的区别？](#51--enum和enum-class的区别)
  - [C++面向对象](#c面向对象)
    ...
    - [6.  复合对象和继承对象，构造函数和析构函数的调用顺序？](#6--复合对象和继承对象构造函数和析构函数的调用顺序)
    - [7.  C++中类成员的访问权限](#7--c中类成员的访问权限)
    - [8.  说一下类的访问权限](#8--说一下类的访问权限)
    - [9.  解释一下this指针](#9--解释一下this指针)
    - [10 . override和overload的区别](#10--override和overload的区别)
    - [11 . 面向对象的3大特性](#11--面向对象的3大特性)
    - [12 . 虚函数和纯虚函数](#12--虚函数和纯虚函数)
    - [13 . 私有继承的作用是什么？](#13--私有继承的作用是什么)
    - [14 . 拷贝构造函数的参数类型必须是引用](#14--拷贝构造函数的参数类型必须是引用)
    - [15 . 什么是多态](#15--什么是多态)
    - [16 . C++中虚函数是如何实现多态的](#16--c中虚函数是如何实现多态的)
    - [17 . 除了虚函数，实现多态还有哪些方式](#17--除了虚函数实现多态还有哪些方式)
    ...
    - [26 . 如何让一个类无法实例化，解释一下这样做有什么应用](#26--如何让一个类无法实例化解释一下这样做有什么应用)
    - [27 . 什么场景下使用继承方式，什么场景下使用组合？](#27--什么场景下使用继承方式什么场景下使用组合)
    - [28 . enum 和 enum class有什么区别？](#28--enum-和-enum-class有什么区别)
...
### 35 . 什么内联函数，说一下`inline`的优缺点，它和宏定义有什么区别？
